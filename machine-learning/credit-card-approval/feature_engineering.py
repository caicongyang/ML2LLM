...
def analyze_feature_importance(data):
    ...
    plt.tight_layout()
    plt.savefig('data/feature_importance_rf.png')
    ...
    plt.tight_layout()
    plt.savefig('data/feature_importance_anova.png')
    ...
    plt.title('特征重要性 (互信息)')
    plt.tight_layout()
    plt.savefig('data/feature_importance_mi.png')
...
def engineer_features(data, output_file='data/engineered_data.csv'):
    ...
    # 然后应用PCA降维（保留95%的方差）
    final_data, pca_transformer = apply_pca(enhanced_data, variance_threshold=0.95)
