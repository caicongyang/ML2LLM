...
def train_model(model, X_train, y_train, X_test, y_test, model_name):
    ...
    plt.savefig(f'data/{model_name.lower().replace(" ", "_")}_confusion_matrix.png')
    ...
    if y_test_prob is not None:
        ...
        plt.grid(True)
        plt.savefig(f'data/{model_name.lower().replace(" ", "_")}_roc_curve.png')
        ...
        plt.grid(True)
        plt.savefig(f'data/{model_name.lower().replace(" ", "_")}_pr_curve.png')
...
def train_multiple_models(X_train, y_train, X_test, y_test):
    ...
    plt.title('模型F1分数比较')
    plt.grid(True)
    plt.savefig('data/model_f1_comparison.png')
    ...
    plt.grid(True)
    plt.savefig('data/model_roc_auc_comparison.png')
    ...
    plt.title('模型训练时间比较')
    plt.grid(True)
    plt.savefig('data/model_training_time_comparison.png')
...
def save_model(model, model_file='data/best_model.pkl'):
    """
    保存模型到文件
...
def train_and_save_best_model(data=None, target_metric='f1'):
    ...
    # 保存最终模型
    final_model = tuned_model if tuned_metric > original_metric else trained_models[best_model_name]
    model_type = "tuned" if tuned_metric > original_metric else "original"
    
    save_model(final_model, f'data/best_model_{best_model_name.lower().replace(" ", "_")}.pkl')
    ...
    return final_model, tuned_metrics if model_type == "tuned" else all_metrics[best_model_name]
