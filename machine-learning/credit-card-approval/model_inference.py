...
def load_model(model_file='data/best_model_random_forest.pkl'):
    """
    加载训练好的模型
...
def load_feature_transformers(file_path='data/feature_transformers.pkl'):
    """
    加载特征转换器
    
    参数:
    file_path (str): 特征转换器文件路径
    
    返回:
    dict: 特征转换器字典
    """
    try:
        transformers = joblib.load(file_path)
        print(f"成功加载特征转换器: {file_path}")
        return transformers
    except Exception as e:
        print(f"加载特征转换器时出错: {e}")
        return None

def load_model_metadata(file_path='data/best_model_metadata.json'):
    """
    加载模型元数据
    
    参数:
    file_path (str): 模型元数据文件路径
...
def visualize_prediction(prediction, probability=None, threshold=0.5):
    ...
    plt.figtext(0.5, 0.01, f'预测结果: {result_text} (置信度: {confidence:.2f})', ha='center', fontsize=12)
    ...
    # 保存图像
    plt.tight_layout()
    plt.savefig('data/prediction_visualization.png')
...
def batch_process_applications(applications_data, model=None, transformers=None, threshold=0.5):
    ...
    if predictions is not None:
        ...
        plt.ylabel('申请数量')
        plt.tight_layout()
        plt.savefig('data/batch_processing_summary.png')
